...
const CountrySelect = ({ selectCountry }) => {
  const [selectedCountry, setSelectedCountry] = useState(selectCountry ? selectCountry.code : "");
  const [selectedFlag, setSelectedFlag] = useState(selectCountry ? selectCountry.flag : "");

  const handleCountryChange = (event) => {
    const selectedCode = event.target.value;
    const selectedCountryData = selectCountry.options.find(country => country.code === selectedCode);
  ...
  return (
    <div className="flex items-center border-none bg-2 rounded-sm h-12">
      <div className="flex items-center bg-1 h-full w-24 p-1 rounded-sm">
        ...
        >
          {selectCountry.options.map((country, index) => (
